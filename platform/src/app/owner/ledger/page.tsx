    ...
    <>
      ...
      <div className="op-wrap">
        ...
        <div className="op-metrics" style={{ marginTop: 18 }}>
          ...
          <div className="op-card">
            <div className="op-label">Cleaning · processing</div>
            <div className="op-figure num">{money(ledger.ytd.cleaning + ledger.ytd.processing)}</div>
            <div className="op-sub num">{money(ledger.ytd.cleaning)} cleaning · {money(ledger.ytd.processing)} fees</div>
          </div>
        </div>

        {ledger.months.length ? ledger.months.map((m) => <MonthTable key={m.key} m={m} money={money} />) : (
        ...
        <div className="op-note" style={{ marginTop: 16 }}>
          Net = gross − cleaning (passthrough) − tax (remitted) − processing fees. Grouped by check-in month.
        </div>
      </div>
    </>
  );
...
function MonthTable({ m, money }: { m: LedgerMonth; money: (n: number) => string }) {
  return (
    <div className="op-card lg-card" style={{ marginTop: 20 }}>
      ...
      <div className="lg-scroll">
        <table className="lg-table num">
          <thead>
            <tr><th className="t">Guest</th><th className="t">Dates</th><th className="t">Status</th><th>Gross</th><th>Cleaning</th><th>Tax</th><th>Fees</th><th>Net</th></tr>
          </thead>
          <tbody>
            {m.rows.map((r) => (
              <tr key={r.id}>
                ...
                <td>{money(r.gross)}</td>
                <td className="dim">−{money(r.cleaning)}</td>
            ...
            <tr className="total">
              ...
              <td className="dim">−{money(m.totals.cleaning)}</td>
